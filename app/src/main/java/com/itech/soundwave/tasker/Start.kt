...
package com.itech.soundwave.tasker
...
class StartActionRunner : TaskerPluginRunnerActionNoOutputOrInput() {
    override fun run(context: Context, input: TaskerInput<Unit>): TaskerPluginResult<Unit> {
        ContextCompat.startForegroundService(
            context,
            Intent(context, SoundWaveService::class.java)
                .setAction(SoundWaveService.ACTION_START)
                .putExtra(SoundWaveService.INTENT_KEY_START_ID, IntegerTable.START_ID_TASKER),
        )
        while (!SoundWaveService.isForeground) {
            Thread.sleep(100)
        }
