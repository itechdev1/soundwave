...
sealed interface Music {
    ...
    private constructor(
        ...
        internal enum class Item(val intCode: Int, val microNamespace: Char) {
            ...
            // part of SoundWave, so these old integer codes remain.
        ...
        private enum class Format(val namespace: String, val microNamespace: Char) {
            /** @see SoundWave */
            SoundWave("com.itech.soundwave", 'a'),
        ...
        companion object {
            /**
             * Creates an SoundWave-style [UID] of random composition. Used if there is no
             * non-subjective, unlikely-to-change metadata of the music.
             *
             * @param item The type of [Item] that created this [UID].
             */
            internal fun SoundWave(item: Item): UID {
                return UID(Format.SoundWave, item, UUID.randomUUID())
            }

            /**
             * Creates an SoundWave-style [UID] with a [UUID] composed of a hash of the non-subjective,
             ...
             *   specification.
             * @return A new SoundWave-style [UID].
             */
            internal fun SoundWave(item: Item, updates: MessageDigest.() -> Unit): UID {
                val digest =
                ...
                return UID(Format.SoundWave, item, uuid)
            }
            ...
            fun fromString(uid: String): UID? {
                if (uid.startsWith("u")) {
                    ...
                    val microFormat =
                        when (uid.getOrNull(1)) {
                            Format.SoundWave.microNamespace -> Format.SoundWave
                ...
                val format =
                    when (split[0]) {
                        Format.SoundWave.namespace -> Format.SoundWave
